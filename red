#!/usr/bin/env bun
// Thin launcher for package-airflow-red. Domain logic lives in the installable
...
const PINS = {
  "package-airflow-red": "github:getcolors/airflow#d1d99882c7b9cc00699afa05e85c2cdf8ecfdad4",
  "package-once-red": "github:getcolors/once#98d3cfa2c743b89a72bac0252c258f9edeedcad7",
...
function installToCache() {
  ...
  const root = join(home, "package-airflow-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-airflow-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let airflow;
try {
  airflow = await import("package-airflow-red");
} catch (err) {
  ...
  const inCheckout = manifest?.name === "package-airflow-red";
  ...
  // to a path.
  airflow = await import(Bun.resolveSync("package-airflow-red", dir));
}

export const run = airflow.run;
if (import.meta.main) await airflow.exec();
